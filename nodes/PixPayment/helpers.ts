...
export function handleMercadoPagoError(error: any): MercadoPagoError {
  ...
  if (error.request) {
    ...
    return {
      message:
        "Erro de conexão com a API do Mercado Pago. Verifique sua conexão com a internet.",
      status: 0,
    };
  }
